...
import se.kth.ict.id2203.epfd.components.Application1b;
import se.kth.ict.id2203.epfd.components.EPFD;
...
public class Assignment1bMain extends ComponentDefinition {
	...
	public Assignment1bMain() {
		...
		trigger(new Application1Init(commandScript, neighborSet, self, this.GAMMA, this.DELTA), epfd
				.getControl());
		...
		connect(flp2p.getNegative(Network.class), network
				.getPositive(Network.class));
		connect(epfd.getNegative(EventuallyPerfectFailureDetector.class), app.getPositive(EventuallyPerfectFailureDetector.class));
	}
