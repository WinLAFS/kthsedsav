...
import yacs.job.events.StateChangeEvent;
import yacs.job.interfaces.StateChangeInterface;
import yacs.job.state.CheckpointInformation;
import yacs.interfaces.*;

public class StateChangeSensor extends yacs.YacsComponent implements SensorInitInterface, StateChangeInterface, BindingController, LifeCycleController {
	...
	public void init(NicheActuatorInterface actuator) {
		...
		this.createYacsLogger( "StateChangeSensor", null, true, true, logger );
	}
	...
	public void reinit(Object[] applicationParameters) {
		log("StateChangeSensor.REinit(Object[]): "  + applicationParameters);
	}


	//////// LoadChangeInterface
	public void checkpoint( ComponentId source, CheckpointInformation information ){
		log("StateChangeSensor.checkpoint("+information.getVersion()+"): " + information);
		information.setSource(source);
		this.trigger.trigger( new StateChangeEvent(source,information) );
		log("StateChangeSensor.checkpoint: event raising done!");
	}
